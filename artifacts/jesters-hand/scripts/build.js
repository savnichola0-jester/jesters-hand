...
// Metro's port is configurable so local test builds can avoid clashing
// with other dev servers (e.g. the mockup sandbox squats on 8081).
...
function exportWebBuild(expoPublicDomain, expoPublicReplId) {
  console.log('Exporting web build (this is what browsers will see)...');
  ...
  if (result.status !== 0) {
    exitWithError('Web export failed — browsers would get no app. Aborting.');
  }
  const indexPath = path.join(projectRoot, 'static-build', 'web', 'index.html');
  if (!fs.existsSync(indexPath)) {
    exitWithError('Web export produced no index.html. Aborting.');
  }
...
async function main() {
  ...
  // Kill Metro before the web export so the two bundlers never fight.
  if (metroProcess) {
    metroProcess.kill();
    metroProcess = null;
  }
