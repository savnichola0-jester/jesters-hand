// Investigation service — builds the admin-only public-activity timeline for
// a searched Joker ID. Pulls every member-visible collection the app writes
// (posts, comments, reactions, tickets, black book, table messages, recruit,
// vault activity, profile) plus the session logs, and merges them into one
// chronological feed.
//
// Whispers are deliberately NOT touched: private conversations stay private.
// Reads rely on the admin's existing rule access (all these collections are
// readable by any active member or by the admin already).
...
export async function fetchInvestigation(
  ...
  const guard = (label: string, p: Promise<void>) =>
    tasks.push(p.catch(() => { failures.push(label); }));
