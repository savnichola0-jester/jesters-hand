              ...
              {tasks.map((t, idx) => (
                <View key={idx} style={s.taskRow}>
                  <TouchableOpacity style={s.taskTypeBtn} onPress={() => {
                    ...
                    <Text style={s.taskTypeText}>{t.type}</Text>
                  </TouchableOpacity>
                  <TextInput style={[s.input, { flex: 1, marginBottom: 0 }]} placeholder="Label" placeholderTextColor="rgba(237,224,196,0.3)" value={t.label} onChangeText={v => setTasks(ts => { const n = [...ts]; n[idx].label = v; return n; })} />
                  <TextInput style={[s.input, { width: 50, marginBottom: 0, textAlign: 'center' }]} keyboardType="number-pad" value={t.targetCount} onChangeText={v => setTasks(ts => { const n = [...ts]; n[idx].targetCount = v; return n; })} />
                  <TouchableOpacity onPress={() => removeTask(idx)}><Feather name="x" size={20} color="#FF6B6B" /></TouchableOpacity>
                </View>
              ))}
...
const s = StyleSheet.create({
  ...
  taskRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 12 },
  taskTypeBtn: { backgroundColor: 'rgba(212,168,83,0.1)', borderWidth: 1, borderColor: GOLD, paddingHorizontal: 8, paddingVertical: 10, borderRadius: 8 },
  taskTypeText: { color: GOLD, fontFamily: 'Cinzel_600SemiBold', fontSize: 10 },
  
  addBtn: { alignItems: 'center', paddingVertical: 12, borderWidth: 1, borderColor: 'rgba(237,224,196,0.2)', borderRadius: 8, marginBottom: 16, borderStyle: 'dashed' },
  addBtnText: { color: 'rgba(237,224,196,0.5)', fontFamily: 'Cinzel_600SemiBold', fontSize: 10 },
